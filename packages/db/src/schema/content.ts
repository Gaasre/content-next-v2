...
export const imageTypeEnum = pgEnum("image_type", ["cover", "content"]);
...
export const articleImage = pgTable(
  ...
  (table) => [
    index("article_image_article_id_idx").on(table.articleId),
    index("article_image_type_idx").on(table.type),
  ]
);
